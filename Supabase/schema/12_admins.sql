-- 12_admins.sql
-- Manages Admin profile and session authentication via typed
-- RPCs and an opaque Postgres sessions table.
-- Fully independent. Safe to run at any time.
...
-- NOTE: DROP TABLE / DROP TYPE / DROP EXTENSION statements belong in reset.sql, not here.
-- Re-running this file only drops and recreates functions (safe to re-run).
...
-- seed user ────────────────────────────────────────────────────────────────────
-- Run this ONCE with real values, then comment it out. Never commit plain credentials.
...
-- SESSIONS TABLE ───────────────────────────────────────────────────────────────
-- Opaque table for holding user sessions.
-- One valid session token per user at a time.
...
-- Raises 'auth.invalid_credentials' if no account matches p_email or the password is wrong.
...
-- Raises 'auth.user_not_found' if no matching row exists.
...
-- ── upsert_session ────────────────────────────────────────────────────────────
-- Updates the existing session for this user (one-session-per-user policy) or
-- inserts a new row if none exists. Returns the full session record.
...
-- Raises 'auth.session_expired' if the session has expired or been deactivated.
-- Updates last_seen_at on every successful validation.
...
-- ── invalidate_session ────────────────────────────────────────────────────────
-- Marks a session as inactive (called by auth/handlers/logout.ts).
