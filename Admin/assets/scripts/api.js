...
async function request(method, path, body=null) {
    ...
    try { apiResponse = await fetch(path, options); } catch {
        responseBody = {
    ...
    if(apiResponse) {
        try { responseBody = await apiResponse.json(); } catch{
            responseBody = {
                success: false, data: null,
    ...
    try{
        ...
        if (!responseBody?.success) {
            if (apiResponse?.status === 401 || responseBody?.error?.code ==='auth.unauthenticated' || responseBody?.error?.code ==='auth.session_expired') {                
                Auth.clearUser();
                Auth.redirectToLogin();
                throw new AppError(
                    responseBody?.error?.code ?? 'auth.unauthenticated', responseBody?.error?.message ??
                    'Session expired! Please sign in before trying again.');
                    return null;
            }
            throw new AppError(responseBody?.error?.code ?? 'unknown', responseBody?.error?.message ?? 'An unexpected error occurred. Please try again.');
            return null;
        }
        ...
        throw new AppError('unknown', error.message ?? 'An unexpected error occurred. Please try again.');
        return null;
    }
}
...
export const api ={
    ...
    contacts: {
        list(params = {}) {
            const qs = new URLSearchParams(
                Object.fromEntries(Object.entries({action: 'list', ...params}).filter(([, v]) => v != null && v !== ''))
            ).toString();
    ...
    email: {
        ...
        retryOutbound({ contact_id, email_type }) {
            return request('POST', '/api/email?action=retry', {
                caller: 'manual',
                contact_id,
                email_type,
            });
